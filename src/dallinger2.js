...
export default dallinger = (function () {
  /**
   * @namespace
   * @alias dallinger
   */
  ...
  dlgr.identity = {
    get recruiter() {
      return dlgr.storage.get("recruiter");
    },
    set recruiter(value) {
      dlgr.storage.set("recruiter", value);
    },
    get hitId() {
      return dlgr.storage.get("hit_id");
    },
    set hitId(value) {
      dlgr.storage.set("hit_id", value);
    },
    get workerId() {
      return dlgr.storage.get("worker_id");
    },
    set workerId(value) {
      dlgr.storage.set("worker_id", value);
    },
    get assignmentId() {
    ...
    get participantId() {
      return dlgr.storage.get("participant_id");
    },
    set participantId(value) {
      dlgr.storage.set("participant_id", value);
    },
    get fingerprintHash() {
      return dlgr.storage.get("fingerprint_hash");
    },
    set fingerprintHash(value) {
      dlgr.storage.set("fingerprint_hash", value);
    },
    get entryInformation() {
      return dlgr.storage.get("entry_information");
    },
    set entryInformation(value) {
      dlgr.storage.set("entry_information", value);
    },
    ...
    initialize: function () {
      ...
      this.uniqueId =
        dlgr.getUrlParameter("workerId") +
        ":" +
        dlgr.getUrlParameter("assignmentId");
  ...
  dlgr.BusyForm = (function () {
    ...
    var BusyForm = function (options) {
      ...
      if (this.target === null) {
        throw new Error(
          'Target HTML element for spinner with ID "' +
            settings.spinnerID +
            '" does not exist.',
        );
      }
  ...
  window.addEventListener("beforeunload", function (e) {
    if (dlgr.preventExit && !dlgr.allowExitOnce) {
      var returnValue =
        "Warning: the study is not yet finished. " +
  ...
  dlgr.goToPage = function (page) {
    if (dlgr.identity.participantId) {
      window.location =
        "/" + page + "?participant_id=" + dlgr.identity.participantId;
    } else {
      window.location =
        "/" +
        page +
  ...
  dlgr.error = function (rejection) {
    ...
    for (var prop in hit_params) {
      if (hit_params.hasOwnProperty(prop))
        add_hidden_input($form, prop, hit_params[prop]);
    }
    if (!rejection.html) {
      $form.submit();
    }
  ...
  dlgr.createParticipant = function () {
    ...
    if (dlgr.identity.entryInformation) {
      ...
      url +=
        ...
        dlgr.identity.mode +
        "?fingerprint_hash=" +
        dlgr.identity.fingerprintHash +
        "&recruiter=" +
        dlgr.identity.recruiter;
    }

    if (
      dlgr.identity.participantId !== undefined &&
      dlgr.identity.participantId !== "undefined"
    ) {
      deferred.resolve();
    } else {
      $(function () {
        ...
        dlgr.post(url, data).done(function (resp) {
          ...
          if (!resp.quorum) {
            // We're not using a waiting room.
            deferred.resolve();
            return;
          }
    ...
    if (
      ...
      $(function () {
        ...
        dlgr.post(url, data).done(function (resp) {
          ...
          dlgr.identity.workerId = resp.participant.worker_id;
          dlgr.identity.assignmentId =
            resp.participant.assignment_id || data.assignment_id;
  ...
  dlgr.waitForQuorum = function () {
    ...
    var socket = new ReconnectingWebSocket(
      ws_scheme +
        location.host +
        "/chat?channel=quorum&worker_id=" +
        dlgr.identity.workerId +
        "&participant_id=" +
        dlgr.identity.participantId,
    );
    var deferred = $.Deferred();
    socket.onmessage = function (msg) {
      if (msg.data.indexOf("quorum:") !== 0) {
        return;
      }
